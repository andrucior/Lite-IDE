...
/** Cold storage / metadata for documents.
  *
  * `contents` and `version` here are point-in-time snapshots: they're authoritative for documents
  * with no active room, but for a live document the truth is in the `DocumentRoom`. Persisting the
  * live state back to here is `save`.
  */
